...
const TopSeller = () => {
 ...
 const  handleFollowed = (index:number)=>{
         ...
         <ul>
          {authors.map((author,index)=>(
            <li key={ index} className="flex items-center justify-between mb-4">
              ...
              <button onClick={()=>handleFollowed(index)} className={`py-1 px-3 rounded  ${author.isFollowing?'bg-red-500 text-white':'bg-black text-white'} `}>{author.isFollowing?"Unfollow":"Follow"}</button>
            </li>
          ))}
         </ul>
    </div>
